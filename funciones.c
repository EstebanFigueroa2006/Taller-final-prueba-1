...
int menuPrincipal(){
    ...
    printf("3.  Editar Zona de Monitoreo\n");
    printf("4.  Eliminar Zona de Monitoreo\n");
    printf("5.  Registrar Medicion de Contaminacion\n");
    printf("6.  Listar Todas las Mediciones\n");
    printf("7.  Listar Mediciones por Zona\n");
    printf("8.  Editar Medicion de Zona\n");
    printf("9.  Eliminar Medicion de Zona\n");
    printf("10. Calcular Promedios Historicos (30 dias)\n");
    printf("11. Comparar con Limites de la OMS\n");
    printf("12. Predecir Niveles Futuros (24 horas)\n");
    printf("13. Generar Alertas Preventivas\n");
    printf("14. Listar Alertas Emitidas\n");
    printf("15. Exportar Reporte General (TXT)\n");
    printf("16. Exportar Reporte a CSV\n");
    printf("17. Visualizar Reportes Guardados\n");
    printf("18. Salir\n");
    printf(">> Seleccione una opcion: ");
    int opc = leerEnteroConRango(1, 18);
    return opc;
}
...
/**
 * Permite editar los datos de una zona existente
 */
void editarZona(){
    Zona zonas[MAX_ZONAS];
    int count = obtenerZonas(zonas);
    ...
    printf("\nSeleccione el numero de zona a editar (0-%d): ", count - 1);
    int idx = leerEnteroConRango(0, count - 1);
    
    printf("\n--- DATOS ACTUALES DE LA ZONA ---\n");
    printf("ID: %d\n", zonas[idx].id);
    printf("Nombre: %s\n", zonas[idx].nombre);
    printf("Ubicacion: %s\n", zonas[idx].ubicacion);
    printf("Poblacion: %d\n", zonas[idx].poblacion);
    ...
    printf("Nuevo nombre de la zona: ");
    leerCadena(zonas[idx].nombre, 50);
    printf("Nueva ubicacion (direccion o coordenadas): ");
    leerCadena(zonas[idx].ubicacion, 100);
    printf("Nueva poblacion aproximada (100-10000000): ");
    zonas[idx].poblacion = leerEnteroConRango(100, 10000000);
    printf("Nuevo tipo de zona (Residencial/Industrial/Comercial/Mixta): ");
    leerCadena(zonas[idx].tipo, 30);
    
    // Guardar todas las zonas actualizadas
    FILE *f = fopen("zonas.dat", "wb");
    if (f == NULL)
    {
        printf("ERROR: No se pudo abrir el archivo de zonas\n");
    ...
    printf("\n[EXITO] Zona editada correctamente\n");
}

/**
 * Permite eliminar una zona de monitoreo
 * También marca como inactivas todas las mediciones de esa zona
 */
void eliminarZona(){
    Zona zonas[MAX_ZONAS];
    int count = obtenerZonas(zonas);
    ...
    printf("\nSeleccione el numero de zona a eliminar (0-%d): ", count - 1);
    int idx = leerEnteroConRango(0, count - 1);
    
    printf("\n[ADVERTENCIA] Esta a punto de eliminar la zona:\n");
    printf("ID: %d - %s\n", zonas[idx].id, zonas[idx].nombre);
    printf("¿Esta seguro? (1=Si, 0=No): ");
    int confirmacion = leerEnteroConRango(0, 1);
    ...
    int idZonaEliminada = zonas[idx].id;
    
    // Eliminar la zona del arreglo (desplazar elementos)
    for(int i = idx; i < count - 1; i++){
        zonas[i] = zonas[i + 1];
    }
    count--;
    
    // Guardar las zonas actualizadas
    FILE *f = fopen("zonas.dat", "wb");
    if (f == NULL)
    {
        printf("ERROR: No se pudo abrir el archivo de zonas\n");
    ...
    // Marcar como inactivas todas las mediciones de esa zona
    Medicion mediciones[MAX_ZONAS * MAX_MEDICIONES_POR_ZONA];
    int numMediciones = obtenerMediciones(mediciones);
    int medicionesAfectadas = 0;
    
    for(int i = 0; i < numMediciones; i++){
        if(mediciones[i].idZona == idZonaEliminada){
            mediciones[i].activa = 0;
            medicionesAfectadas++;
        }
    }
    
    // Guardar las mediciones actualizadas
    if(medicionesAfectadas > 0){
        FILE *fm = fopen("mediciones.dat", "wb");
        if (fm != NULL)
        {
            fwrite(mediciones, sizeof(Medicion), numMediciones, fm);
            fclose(fm);
        }
    }
    
    printf("\n[EXITO] Zona eliminada correctamente\n");
    printf("[INFO] %d mediciones asociadas fueron marcadas como inactivas\n", medicionesAfectadas);
}
...
/**
 * Permite editar una medición de contaminación de una zona específica
 */
void editarMedicionDeZona(){
    ...
    printf("\n--- EDITAR MEDICION DE CONTAMINACION ---\n");
    
    // Seleccionar zona
    listarZonas(zonas);
    printf("\nSeleccione el numero de zona (0-%d): ", numZonas - 1);
    int idxZona = leerEnteroConRango(0, numZonas - 1);
    int idZona = zonas[idxZona].id;
    
    // Mostrar mediciones de esa zona
    printf("\n--- MEDICIONES DE LA ZONA: %s ---\n", zonas[idxZona].nombre);
    int medicionesZona = 0;
    int indices[MAX_MEDICIONES_POR_ZONA];
    
    for(int i = 0; i < numMediciones; i++){
        if(mediciones[i].idZona == idZona && mediciones[i].activa == 1){
            printf("[%d] ID:%d | Fecha: %02d/%02d/%d | PM2.5=%.1f PM10=%.1f CO2=%.0f\n",
                   medicionesZona,
                   mediciones[i].idMedicion,
                   mediciones[i].dia,
                   mediciones[i].mes,
                   ...
                   mediciones[i].niveles.pm10,
                   mediciones[i].niveles.co2);
            indices[medicionesZona] = i;
            medicionesZona++;
        }
    }
    
    if(medicionesZona == 0){
        printf("No hay mediciones activas para esta zona\n");
        return;
    }
    
    printf("\nSeleccione el numero de medicion a editar (0-%d): ", medicionesZona - 1);
    int seleccion = leerEnteroConRango(0, medicionesZona - 1);
    int idx = indices[seleccion];
    
    printf("\n--- EDITANDO MEDICION ID: %d ---\n", mediciones[idx].idMedicion);
    
    // Fecha de la medición
    printf("\nFecha actual: %02d/%02d/%d\n", mediciones[idx].dia, mediciones[idx].mes, mediciones[idx].anio);
    printf("Nueva fecha:\n");
    printf("Dia (1-30): ");
    mediciones[idx].dia = leerEnteroConRango(1, 30);
    printf("Mes (1-12): ");
    mediciones[idx].mes = leerEnteroConRango(1, 12);
    printf("Anio (1996-2026): ");
    mediciones[idx].anio = leerEnteroConRango(1996, 2026);
    
    // Datos climáticos
    printf("\n--- DATOS CLIMATICOS ---\n");
    printf("Temperatura actual: %.1f°C\n", mediciones[idx].clima.temperatura);
    printf("Nueva temperatura (°C) (-20 a 50): ");
    mediciones[idx].clima.temperatura = leerFlotanteConRango(-20, 50);
    
    printf("Velocidad viento actual: %.1f km/h\n", mediciones[idx].clima.velocidadViento);
    printf("Nueva velocidad del viento (km/h) (0-150): ");
    mediciones[idx].clima.velocidadViento = leerFlotanteConRango(0, 150);
    
    printf("Humedad actual: %.1f%%\n", mediciones[idx].clima.humedad);
    printf("Nueva humedad (%%) (0-100): ");
    mediciones[idx].clima.humedad = leerFlotanteConRango(0, 100);
    
    // Niveles de contaminantes
    printf("\n--- NIVELES DE CONTAMINANTES ---\n");
    printf("PM2.5 actual: %.1f μg/m³\n", mediciones[idx].niveles.pm25);
    printf("Nuevo PM2.5 (μg/m³) (0-500): ");
    mediciones[idx].niveles.pm25 = leerFlotanteConRango(0, 500);
    
    printf("PM10 actual: %.1f μg/m³\n", mediciones[idx].niveles.pm10);
    printf("Nuevo PM10 (μg/m³) (0-600): ");
    mediciones[idx].niveles.pm10 = leerFlotanteConRango(0, 600);
    
    printf("CO2 actual: %.0f ppm\n", mediciones[idx].niveles.co2);
    printf("Nuevo CO2 (ppm) (300-5000): ");
    mediciones[idx].niveles.co2 = leerFlotanteConRango(300, 5000);
    
    printf("SO2 actual: %.1f μg/m³\n", mediciones[idx].niveles.so2);
    printf("Nuevo SO2 (μg/m³) (0-500): ");
    mediciones[idx].niveles.so2 = leerFlotanteConRango(0, 500);
    
    printf("NO2 actual: %.1f μg/m³\n", mediciones[idx].niveles.no2);
    printf("Nuevo NO2 (μg/m³) (0-400): ");
    mediciones[idx].niveles.no2 = leerFlotanteConRango(0, 400);
    
    // Factores sociales y culturales
    printf("\n--- FACTORES SOCIALES Y CONTEXTUALES ---\n");
    printf("Densidad trafico actual: %s\n", mediciones[idx].factoresSociales.densidadTrafico);
    printf("Nueva densidad de trafico (Alta/Media/Baja): ");
    leerCadena(mediciones[idx].factoresSociales.densidadTrafico, 20);
    
    printf("Dia festivo actual: %s\n", mediciones[idx].factoresSociales.esDiaFestivo ? "Si" : "No");
    printf("Es dia festivo? (0=No, 1=Si): ");
    mediciones[idx].factoresSociales.esDiaFestivo = leerEnteroConRango(0, 1);
    
    printf("Hora actual: %d\n", mediciones[idx].factoresSociales.horaDelDia);
    printf("Nueva hora del dia (0-23): ");
    mediciones[idx].factoresSociales.horaDelDia = leerEnteroConRango(0, 23);
    
    printf("Estacion actual: %s\n", mediciones[idx].factoresSociales.estacion);
    printf("Nueva estacion (Verano/Invierno/Primavera/Otono): ");
    leerCadena(mediciones[idx].factoresSociales.estacion, 20);
    
    // Guardar todas las mediciones actualizadas
    FILE *f = fopen("mediciones.dat", "wb");
    if (f == NULL)
    {
        printf("ERROR: No se pudo abrir el archivo de mediciones\n");
        return;
    }
    fwrite(mediciones, sizeof(Medicion), numMediciones, f);
    fclose(f);
    
    printf("\n[EXITO] Medicion editada correctamente\n");
}

/**
 * Permite eliminar (marcar como inactiva) una medición de una zona específica
 */
void eliminarMedicionDeZona(){
    ...
    printf("\n--- ELIMINAR MEDICION DE CONTAMINACION ---\n");
    ...
    printf("\nSeleccione el numero de medicion a eliminar (0-%d): ", medicionesZona - 1);
    int seleccion = leerEnteroConRango(0, medicionesZona - 1);
    int idx = indices[seleccion];
    
    printf("\n[ADVERTENCIA] Esta a punto de eliminar la medicion:\n");
    printf("ID: %d | Fecha: %02d/%02d/%d\n",
           mediciones[idx].idMedicion,
           mediciones[idx].dia,
           mediciones[idx].mes,
           mediciones[idx].anio);
    ...
    // Marcar la medición como inactiva
    mediciones[idx].activa = 0;
    ...
    printf("\n[EXITO] Medicion eliminada correctamente\n");
}
